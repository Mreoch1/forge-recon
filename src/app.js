...
app.post('/delete-demo-data', async (req, res) => {
  ...
  try {
    ...
    const result = { found: [], notFound: [], errors: [], projectsDeleted: [], customersDeleted: [] };

    // Delete jobs/projects by title
    const projectTitles = ['test', 'Demo Renovation - Building B', 'Demo Renovation - Building A'];
    for (const title of projectTitles) {
      const { data: jobs } = await sb.from('jobs').select('id, title, customer_id').ilike('title', title);
      for (const job of (jobs || [])) {
        // Find all WOs for this job
        const { data: wos } = await sb.from('work_orders').select('id, display_number').eq('job_id', job.id);
        for (const wo of (wos || [])) {
          // Delete estimates/invoices for each WO
          ...
          for (const est of (ests || [])) {
            ...
            await sb.from('estimates').delete().eq('id', est.id);
          }
        ...
        // Delete job members, SOV, decisions, RFPs, change orders
        await sb.from('project_members').delete().eq('job_id', job.id);
        await sb.from('project_sov_items').delete().eq('job_id', job.id);
        await sb.from('project_decisions').delete().eq('job_id', job.id);
        await sb.from('project_rfps').delete().eq('job_id', job.id);
        // Delete change_orders if table exists
        try { await sb.from('change_orders').delete().eq('job_id', job.id); } catch(e) {}
        // Delete job itself
        await sb.from('jobs').delete().eq('id', job.id);
        result.projectsDeleted.push({ title: job.title, id: job.id });
      }
    }

    // Delete DEMO customers
    const demoCustomers = ['Sample Customer A (DEMO)', 'Sample Customer B (DEMO)'];
    for (const cname of demoCustomers) {
      const { data: custs } = await sb.from('customers').select('id, name').ilike('name', cname);
      for (const c of (custs || [])) {
        await sb.from('customers').delete().eq('id', c.id);
        result.customersDeleted.push({ name: c.name, id: c.id });
      }
    }

    // Then delete WOs by display_number (as before)
    for (const dn of displayNumbers) {
      const cleanDn = dn.replace(/^WO-/, '');
      const { data: wo } = await sb.from('work_orders').select('id, display_number').eq('display_number', cleanDn).maybeSingle();
