...
(function () {
  ...
  if (!document.getElementById('ai-chat-styles')) {
    ...
    style.textContent = `
      ...
      #ai-chat-root, #ai-chat-root * { max-width: 100vw; }
      .recon-aic-head {
      ...
      @media (max-width: 640px) {
        ...
        .recon-aic-panel {
          ...
          width: calc(100vw - 16px); min-width: 0; max-width: calc(100vw - 16px);
        ...
        .recon-aic-input {
          ...
          padding: 8px; gap: 6px; width: 100%; max-width: 100%;
        }
        .recon-aic-input textarea {
  ...
  async function send(text) {
    ...
    try {
      ...
      const errorMsg = {
        role: 'assistant',
        content: isTimeout
          ? "FORGE is having trouble right now â€” the request took too long."
      ...
      fetch('/ai/chat-errors', {
        ...
        if (data && data.err_id) {
          ...
          if (lastMsg && lastMsg.error && lastMsg === errorMsg) {
